...
```
 ...
 ├──────────────────────────────────────────┤
 │                                          │
 │ 一句话：返回 Counter（dict 子类），        │
 │   key=元素 value=出现次数；常用来          │
 │   统计列表/字符串中各元素的出现频率。      │
 │                                          │
 │ 签名：`collections.Counter(               │
 │         iterable=None, /, **kwds          │
 │       ) -> Counter`                       │
 │                                          │
 ├──────────────────────────────────────────┤
...
```
 ...
 ## 参数
 │ 参数     │ 类型     │ 必填 │ 默认 │ 说明           │
 │ iterable │ Iterable │ 否   │ None │ 任意可迭代对象 │

 ## 注意
 • Counter 继承自 dict，但访问不存在的键时返回 0 而非 KeyError
 • 传入空迭代器不会报错，会得到空的 Counter()
 • 包含 unhashable 类型（如 list）会抛 TypeError
```
